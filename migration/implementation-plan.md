...
- **Next:** the two new Guard cases pass locally against the existing binaries; rebuilding Rust locally hit `ENOSPC` on the external target volume, so native CI must verify them on fresh binaries. Commit the ledger/checkpoint/test, then rerun the required gates on that final integrated SHA. SEC-005 remains partial native and release-only rows stay non-green without separate authorization.
