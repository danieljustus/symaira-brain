...
## Resume checkpoint — 2026-09-24 (v0.12.0 distribution evidence)

- **Candidate:** `codex/dist-evidence-v0120-20260924` starts from integrated `main` at `dc9c54e41beccf131fbe70e3f45bfff98871e709`. The separate `release/v0.12.1` checkout at `d8fbb14d` has successful exact-SHA CI; PR #677 was merged earlier. This checkpoint does not promote DIST-001/002 or authorize publication/cutover.
- **Current published Go release:** `v0.12.0` is now published. The pinned GitHub API capture has 40 assets; the pinned Homebrew tap commit is `9e3f8ecbda2025becf49a623645707e18c977260`. `go run ./scripts/dist-oracle -check` passes 44/44 against the new fixture. The read-only `verify-release.py --verify-signatures --verify-sboms` over all downloaded assets passes 40 size/digest checks, 12 checksum entries, 13 Cosign signatures with workflow/OIDC/transparency verification, six archive file sets, six CycloneDX SBOMs parsed by Syft, and four formula plus one cask release links. Five verifier negative tests and the Go package tests pass. Downloaded assets and tap bytes are preserved at `/Volumes/1TB_NVMe_SN850X/Dev/symbrain-dist-v0.12.0.8khxgT`.
- **Residual:** these are receipts for the existing **Go** release, not Rust distribution evidence. `.goreleaser.yml` still builds `./cmd/symbrain`; DIST-001/002 remain pending until Rust candidate archives and checksums are verified for all six targets and their applicable distribution contract is exercised. Signing/publication, Homebrew writes and cutover require separate authorization. No release, Go removal or destructive cleanup was performed.
...
- **SEC-005 candidate:** the existing Go-compatible JSON syntax scanner now handles malformed audit anchors natively; a narrow TOML missing-equals class handles the invalid-config scenario natively. Unsupported parser/type/read errors still gate before output. The frozen Go doctor oracle has nine cases, all nine pass through the native Rust binary on macOS. Independent read-only reviews found no actionable parity defects.
- **Local executable receipts:** pinned Go 1.26.7 oracle `-check` reports zero drift across nine cases; the focused Rust integration and six doctor unit tests pass; the full CLI differential reports 459 passing cases on macOS, including both newly native doctor errors. `cargo fmt --check`, `gofmt`, Python syntax and `git diff --check` pass. An unpinned Go 1.27.1 differential run produced two unrelated harness JSON-diagnostic differences and is discarded; the repository-pinned Go 1.26.7 run passed.
- **Ledger reconciliation:** SEC-005 is promoted from five of seven native cases to nine of nine on Linux, macOS, and Windows. The independent review's suspected `foo/..` path defect did not reproduce; a direct Rust `PathBuf::pop` probe confirmed the behavior and a regression assertion now covers the full cleaner input. The matrix has 72 rows: 70 green and only DIST-001/002 pending.
- **Windows broker gate:** CI on ledger commit `4b7cb469` passed the Windows doctor oracle, CLI differential, and Rust workspace, but `internal/broker.TestClient_CallAfterClose_ReturnsClosedError` timed out in the portable broker step. The Go read loop exposed `Done()` before setting `pending = nil`, letting a new call block writing to an unconsumed pipe. The close-state ordering is fixed; pinned Go 1.26.7 passes 100 focused repetitions and the full broker race suite locally. The final candidate still needs all target gates.
- **Remaining gates:** run CI on the broker fix, integrate PR #677, then verify the exact merged `main` SHA. DIST-001/002 still require a real current release with signed assets, SBOM, Homebrew and install receipts; `v0.12.0` is absent. Preserve the existing worktrees and build caches. Storage exhausted during this slice: the internal data volume rejected small source writes, so follow-up work uses the isolated external worktree `.worktrees/codex-sec005-path-clean-20260924` from `620e8625`; no cache was removed.

## Resume checkpoint — 2026-09-23 (final integrated candidate)

- **Mode:** execute; goal active. Draft PR [#674](https://github.com/danieljustus/symaira-brain/pull/674) uses `codex/migration-integrated-20260923`. The branch includes source fixes, the reconciled matrix and two Guard fallback differential cases. `main` remains untouched. No publication, cutover, Go removal, paid provider, or destructive cleanup is authorized.
- **Integrated source receipts:** [CI 35908259861](https://github.com/danieljustus/symaira-brain/actions/runs/35908259861) passed every job on `4c699b46`, including Linux/macOS Rust gates, native Linux/macOS/Windows differential gates, the GoReleaser snapshot and GUI Go/Rust binary contracts (one passed, zero failed/skipped for each binary on Xcode 26.6). The native Windows `symbrain-managed` suite executed six install tests, including `.exe` version and timeout probes, with zero skips.
- **Ledger reconciliation:** `migration/contract-matrix.csv` has 72 distinct rows. Edits record target-platform receipts for CLI-006A/007/009/010, MCP-001..005, SEC-001, SET-001, GUI-001, INS-002, ADR-001..004, SKL-002/003 and USE-001. SEC-005 has five native Go-oracle matches out of seven; malformed TOML and JSON anchor diagnostics deliberately gate to Go before output because exact parser error text has no native port. The new two-case CLI differential proves the fallback's exact bytes and exit status locally; native CI on the final SHA is still required.
- **External boundaries:** DIST-001/002 have an offline `dist-oracle` (44/44) and six staged GoReleaser archives/checksums, while current signed release assets, SBOM, Homebrew and install receipts require a real release. External NVMe free space fluctuated from roughly 30 MiB to 5 GiB; preserve all worktrees and build data.
- **Next gate:** the two new Guard cases pass locally against the existing binaries; rebuilding Rust locally hit `ENOSPC` on the external target volume, so native CI must verify them on fresh binaries at the current integrated HEAD. SEC-005 remains partial native and release-only rows stay non-green without separate authorization.
...
- **Verified at `2c7eac60`:** CI runs `35864722105` (CI) and `35864722125` (Rust guard) concluded success for that exact SHA. Linux and macOS `rust-migration` execute `make rust-check parity-smoke`; Windows currently runs audit/guard, init and selected skills tests, not the whole ledger. Locally, `go run ./scripts/secret-oracle -check` passed **30** Go cases on this candidate source, and `cargo test -p symbrain-usage --test secret_oracle_tests --locked` passed **9** Rust tests. `make usage-oracle-check usage-opencode-check`, `cargo test -p symbrain-usage --locked` (**25 + 9 + 8** tests) and strict all-target usage Clippy passed. All local commands used `scripts/run-external-env.sh` on the mounted NVMe. A first filtered cargo invocation executed zero tests and is explicitly discarded.
- **MCP row correction at the same SHA:** `make gateway-oracle-check mcp-oracle-check` passed **8** gateway and **20** framing cases; `cargo test -p symbrain-gateway --locked` and `cargo test -p symbrain-mcp --locked` exercised the matching Rust suites. MCP-001/002/004 previously cited `scripts/mcp-oracle`, whose 20 cases only cover framing. Their actual initialize/list/stdout Go black-box fixture is `scripts/gateway-oracle`, now named in the matrix. MCP-003 remains partial because the Go fixture covers routing/errors while connection cancellation and Windows runtime are not jointly proven.
- **Active isolated lanes:** GUI-001 in `.worktrees/codex-gui-001-20260923` (base `2c7eac60`); DIST-001/002 in `.worktrees/codex-dist-001-20260923` (open PR #659 head `702eede9`, merged `2c7eac60` as `c12953c4`). DIST's offline oracle was independently rerun: **44/44** assertions, while v0.12.0 has no published release and the latest v0.11.0 release has 40 captured assets. Live byte/signature checks are in progress. A separate Windows target-evidence audit is in progress. None of these lanes is promoted on its own branch result.
- **Residuals:** SEC-001 and USE-001 have no native Windows runtime receipt for their full rows. The `all`-platform matrix rows need a row-by-row current-candidate target audit. DIST signing/SBOM/Homebrew asset checks and GUI integration remain open. Authenticated live Usage paths still use the Go fallback; no real credentials or paid endpoints are authorized for this run. Release, cutover, Go removal and destructive cleanup are outside scope.
- **Next:** integrate independently reviewed lanes, run affected checks and the complete applicable ledger gates on the integrated candidate, obtain native target evidence, then update row statuses from receipts. Preserve all pre-existing worktrees and WIP.

## Resume checkpoint — 2026-09-22 (wave results): SEC-002 merged, SEC-001 slice 1 in review, #621 verdict + fix slice

- **SEC-002 merged as #650** (squash `85d528c4`; CI green on linux/macos/windows incl. `rust-migration`, `rust-migration-macos`, `rust-init-native` (ubuntu+windows), `stdio-hygiene`, `rust-windows-audit-guard`). Coordinator verification, not worker claims: `go run ./guard/scripts/guard-oracle -check` x2 -> `0 drift on 164 cases`; `go test ./guard/...` ok; `gofmt -l`/`go vet` clean; `cargo test -p symbrain-guard-core` 41 pass; `cargo test -p symbrain-cli guard` 14 pass; `cargo clippy --all-targets -- -D warnings` 0 on both crates; `cargo fmt --all --check` clean; scope = 12 files (oracle generator + guard-core + one cli test); base-checked. Matrix row SEC-002 promoted: `go_oracle` corrected to `guard/scripts/guard-oracle` (the generator that actually pins the bytes), `rust_test` now names the two real test paths, comparison set to `exact-json-and-errors` (no file-manifest test exists in this slice).
- **Dispatch lesson, confirmed twice in one session:** the first wave's SEC-001 and #621 children spent their whole budget in read-only discovery and wrote nothing; the narrowed re-dispatch (transcript findings pre-seeded into the brief, commit-first instruction) delivered both. Future briefs must fold prior-transcript findings in and demand an early commit.
- **SEC-001 slice 1 (resolution) -> PR #652 open.** New `scripts/secret-oracle` (27 cases; drives the real `internal/memory/secrets` + corekit `secretref` against a fake `symvault` on PATH, throwaway HOME/XDG) plus the native match in `symbrain-usage`. Coordinator reran every acceptance gate before and after rebase onto `85d528c4` (oracle x2, 36 cargo tests, clippy, fmt, gofmt, vet). Redaction = slice 2. Recorded boundaries: lossy non-UTF-8 secret stdout (now issue **#653**), Rust stdout cap (hardening), `keychain://` fixture exclusion. The matrix row stays `pending` until #652 merges.
- **#621 verdict: still-broken.** All five reported symptoms reproduced on post-#647 main; evidence comment posted on the issue. The Rust-parity part (native sync resolves `symbrain/skills` while Go's `internal/paths.resolve` prefers the legacy `symskills` root when it exists — regression new since #647) is dispatched as a fix slice in `.worktrees/i621-drift`. Go-side design gaps stay owner decisions on the issue: no `pull` verb behind the `use symskills pull` hint, `--force` undefined on Go and silently ignored in the shipped binary, base snapshots never advance, no render->library import.
- **Closeout:** superseded provisioned worktrees (`sec001-secret`, `sec002-guard`) and the dead first-wave child worktrees/branches without commits are removed; `subagent-sa-0-453f1033` (PR #652) and the #621 evidence worktree are kept while their PRs/issues are open; the fix child's worktree stays until integration.
- **Promotion gates run on `85d528c4`** (disjoint crates; the run predated the `1ee25ba8` merge and is unaffected by it): `go test ./internal/harness/...` ok; guard/instructions/adapters/skills/skills-install oracles `-check` all PASS (164/32/25/-/11 cases); `cargo test -p symbrain-harness -p symbrain-instructions -p symbrain-adapter -p symbrain-skills` all suites ok; the tree stayed clean (generators wrote nothing). 13 `fixture-ready` rows promoted (HAR-001..006, SEC-003, SEC-004, INS-001, ADR-005, SKL-001..003) after verifying each row's `rust_test` path exists; `DOC-001` stays fixture-ready until the `scripts/rust-differential.py` doctor cases run (`make parity-smoke` in flight).
### Resume checkpoint — 2026-09-22 evening (manual stop requested)

**Merged today, coordinator-verified:** #650 SEC-002 (`85d528c4`) · #652 SEC-001 slice 1 (`1ee25ba8`, merged red — see correction above) · #656 secret-oracle compiled fake (`918f9eb0`) · (externally) #657 CoreKit evidence (`7ea853d3`). Main was CI-green at `7ea853d3`.

**Dispatch wave `deleg_8c94acdb` results (all three children verified by the coordinator, not by their claims):**
- **Slice 2 / SEC-001 redaction — DONE, PR #658 open** (`be2578b4`): value_leaks object per failed case from real Go runs, generation fails loud if any channel carries a plaintext (pins `resolve.go:44`), +2 post-value failure probes, counts 27->29. My runs: oracle -check x2 PASS (29), gofmt/vet/security ok, usage tests+clippy+fmt clean, scope exactly 3 files. Side finding: `provider_errors.rs::redact()` has no Go twin in repo or corekit — Rust-only hardening, out of this row's scope, recorded here.
- **DIST-001/002 — DONE, PR #659 open** (`702eede9`): offline `scripts/dist-oracle -check` gate, golden manifest of the real v0.11.0 release (40 assets), static goreleaser assertions. My runs: check exit 0, corrupted-manifest self-test exit 1, restored 0, gofmt/vet/security ok, scope only `scripts/dist-oracle/**`. Six honest `not-run:` lines (cosign bytes, rebuild, syft regen, brew install, digest recompute, live fetch).
- **GUI-001 — DONE, verified, NOT pushed, NO PR** (`25709be7`, base `7ea853d3`): only `Tests/SymBrainCoreTests/ModelTests.swift` +114; child found NO Rust schema gap (version/doctor/profile/sync JSON byte-identical Go vs Rust after path normalization; env switch `SYMBRAIN_TEST_BINARY` else `$HOME/.symbrain-test-binary` because xcodebuild drops env to xctest). My runs: scope/forbidden/tree clean; full SymBrainCore suite against the Rust binary `TEST SUCCEEDED` exit 0 in 38.9s. First GUI dispatch died mid-research empty; this is the re-dispatch (`deleg_7ce9d4cd`).
- Also today: issue #621 comment (item 4 fix in #655, item 3 reclassified as Go design), issue #480 comment (skills_targets_native /private/var evidence), issue #653 (non-UTF-8 secret bytes), matrix CSV structural repair, all 14 fixture-ready rows promoted, DOC-001 green (parity-smoke 457/457 incl. doctor cases).

**Exact next actions on resume (in order):**
1. `git fetch` on main; for each of #654 (head `a0496d28`), #655 (`87b44e25`), #658, #659: `gh pr checks <n> | grep -v $'\tpass\t'` must be empty; if BEHIND, `git merge origin/main` in that branch again (up-to-date rule blocked merges once today), rerun focused gates, push; then squash-merge, read back, `git pull --ff-only` on main. CI runs continue server-side — the four local watches were stopped at the manual stop.
2. Push `hermes-subagent/subagent-sa-0-1e4cc990` (25709be7), open PR, watch, merge (already coordinator-verified).
3. Promotions on this docs branch after the respective merges: SEC-001 -> `green-29-oracle-cases-resolution-and-redaction`; DIST-001/002 -> green with the not-run list kept in the status/ledger; GUI-001 -> `green-115-swift-tests-both-binaries-json-byte-identical`; note #621 item 4 closed via #655.
4. Closeout: remove merged/dead worktrees+branches (i621-drift unused, subagent-sa-1-8e033e27 evidence worktree after #621 follow-up, sa-1-c48b1172 already merged), then Phase 12 / remaining gaps (GCLI-PLAT-01 row state, open promise: usage row #620 Teil 2 OpenCode gaps per ledger).

**Session lessons already written into this skill:** CI watches must run unpiped; children don't push — push their branches yourself; `gh pr merge` fails behind the up-to-date rule — integrate main first.

- **Wave dispatched (`deleg_8c94acdb`, base `origin/main@918f9eb0`):** (A) SEC-001 slice 2 redaction — child `sa-0-893cc031`: Go-generated value-leak invariant on `scripts/secret-oracle`, plus a report-not-invent hunt for the Go origin of `rust/symbrain-usage/src/provider_errors.rs::redact()` (exact-phrase greps of repo+corekit already negative). (B) DIST-001+002 — child `sa-1-e59964aa`: golden artifact manifest pinned from the real v0.11.0 release (40 assets) + `scripts/dist-oracle -check` gate over `.goreleaser.yml`; `not-run` lines mandatory for anything requiring cosign/builds. (C) GUI-001 — child `sa-2-6d42f3b3`: SymBrainCore test suite against BOTH Go and Rust binaries for version/command JSON schemas; Rust-side fixes only, no weakened Swift assertions. All three write only in their runtime worktrees; no file overlap; no GitHub writes by children.
- **Correction (same day, factual):** #652 was merged with `go-test-shard (2)` RED. `internal/security.TestNoShellInterpolation` flagged `scripts/secret-oracle/main.go:86` (the shell-script fake symvault); the CI watch had been piped through `tail`, so `gh pr checks --watch --fail-fast`'s own exit status was masked and only `tail`'s 0 was observed. Main's push run for `1ee25ba8` (`35739040256`) was `failure`. Fixed in **#656** (compiled fake helper with JSON sidecar; the 27-case fixture stayed byte-identical), merged as `918f9eb0`; the verifiable facts are: `go test -count=1 ./internal/security/` ok on the fix branch, `gh pr checks 656` all-pass unpiped before merge, and the push run for `918f9eb0` on main. Standing rule now recorded in the skill: never pipe a CI watch; print non-pass checks before merging.
- **DOC-001 promoted:** `make parity-smoke` exited 0 with `457 parity cases passed`; its recipe runs `python3 scripts/rust-differential.py <go> <rust>`, whose corpus contains the `doctor_*` JSON/human/flag cases (16 doctor mentions: `doctor_empty_json`, `doctor_empty_human`, `doctor_global_json`, `doctor_help`, ...). Row is now `green-457-parity-cases-doctor-json-and-human`. With that, no `fixture-ready` row remains.
- **Update same day:** #652 merged after green CI on all platforms (squash `1ee25ba8`); SEC-001 promoted to `partial-resolution-native-27-oracle-cases-redaction-pending` with its real oracle/test paths.
- **Next:** merge #652 -> promote SEC-001 as `partial` (resolution native, redaction pending); integrate the #621 fix slice; run the 14 `fixture-ready` promotion gates; Phase 11 (`DIST-001`, `DIST-002`, `GUI-001`) untouched.

## Resume checkpoint — 2026-09-22 (reconciled): skills Slices A+B are on main; next open rows SEC-001/SEC-002

**Base: main `fadfdab9`** (2026-09-21 22:51, `#647`), coordinator checkout clean at session start.

- **Reconciled, not rediscovered.** The skills-runner slices landed as `#644` (runner port), `#645` (on-disk freeze), `#646` (render-dir symlink target) and `#647` (native `sync`), so the "verify Slice A, then Slice B" next action and the `guard doctor` next action are both complete on this revision; `#624` (atime flake) is closed. Go SEC-001 baseline measured here: `go test ./internal/memory/secrets/...` passes.
- **Stale worktree retired with evidence.** `.worktrees/w-install-disk` (`migration/install-disk-oracle-20260921`) was verified fully superseded: its committed content is a subset of main (squash-merged as `#645`/`#646`) and its uncommitted diff held only the pre-refinement `match` form of the `render_dir` fix that main carries as `if let`. Diff archived to the session scratch dir; worktree and branch removed.
- **Task order for this session (dependency order):**
  1. `SEC-001` secret-reference resolution and redaction — Go oracle from `internal/memory/secrets/resolve.go` + `internal/memory/security/jwt.go`; existing ad-hoc Rust `symvault://` handling lives in `rust/symbrain-usage/src/provider_config.rs`, `rust/symbrain-cli/src/doctor_cli.rs`, `rust/symbrain-cli/src/doctor_links.rs`.
  2. `SEC-002` guard decisions/grants/audit chain — `guard/internal/{approval,grant,audit}`, oracle generator `guard/scripts/guard-oracle`, Rust side `rust/symbrain-guard-core` (`tests/oracle_tests.rs` already exists for SEC-003/004 — extend it, do not fork it).
  3. `#621` skills-status drift: re-verify against post-`#647` main before any fix; close with evidence or fix in its own slice.
  4. Promote `fixture-ready` rows only by running their gates (`DOC-001`, `SEC-003/004`, `HAR-001..006`, `INS-001`, `ADR-005`, `SKL-001..003`); the coordinator edits the matrix.
  5. Phase 11 (`DIST-001`, `DIST-002`, `GUI-001`) remains the largest open block.
- **Loaded this session — do not reload:** `go-to-rust-migration` + `references/worker-dispatch.md`, `go-rust-port-parity` + `references/workflow.md`, `guard-repo`, `autonomous-coding-agents`, `parallel-repo-agents`, `code-editing`.
- **Wave 20260922:** three isolated worktrees off `fadfdab9` — `migration/sec001-secret-resolution-20260922` (`.worktrees/sec001-secret`), `migration/sec002-guard-decisions-20260922` (`.worktrees/sec002-guard`), `migration/i621-skills-drift-20260922` (`.worktrees/i621-drift`). `make rust-check` measured GREEN on `fadfdab9` (exit 0, 415 s wall, ends `advisories ok, bans ok, licenses ok, sources ok`). Matrix: only a CSV-structural repair in this checkpoint — lines 13 (`CLI-006B`) and 38 (`CFG-001`) had unquoted commas inside `fixture_or_input`, so strict parsers shifted columns (e.g. `CFG-001.status` read as `scripts/xdg-oracle`); both fixture fields are now quoted, content byte-identical to HEAD, verified: 72 rows x 8 fields, 0 malformed, status census unchanged (14 fixture-ready, 5 pending). No row status was promoted.
- **Still not authorized:** Go removal, tag/publish/release, pushing unverified work, claiming `fixture-ready` rows green.
...
`make mcp-oracle-check` → `PASS: MCP oracle deterministic check passed (20 cases)`; `make gateway-oracle-check` → `PASS … (8 cases)`; `make rust-fuzz-smoke` → `Done 10000 runs`, exit 0; `cargo test -p symbrain-mcp` → 16 protocol tests + 2 differential tests green; `cargo test -p symbrain-gateway` → 8 tests, of which `fixture_is_the_complete_gateway_black_box_contract` iterates **all eight** fixture cases and asserts stdout bytes, the error classifications and `stdout_non_protocol_bytes == 0`.

**What the fixtures actually contain was checked, not assumed.** The 20 MCP oracle cases are framing/model level only (`line_ping`, `line_*`, `framed_*`, `invalid_length_*`, `missing_length`, `partial_body`, `oversized_line`, `max_framed`, malformed/null-id/notification) — they do **not** exercise initialize, tools/list or tools/call. So MCP-001…MCP-004 are *not* promoted on "the tests pass": they now read `partial-native-16-protocol-tests-no-go-byte-fixture`, because their semantics are covered by Rust protocol tests with no Go byte fixture behind them. GW-001…GW-004 do cover those semantics (`initialize-capabilities`, `tools-list-catalog`, `tools-call-routing`, `tools-call-tool-error`, `tools-call-hidden`, `tools-call-bootstrap`, `tools-call-patterns`, `stdout-hygiene`) and are promoted to `green-native-8-of-8-go-oracle-black-box-cases` (GW-004 additionally on CI `stdio-hygiene`). MCP-000 → `green-native-20-of-20-go-oracle-cases-byte-exact`, MCP-FUZZ → `green-native-10000-fuzz-runs-both-targets`.

**Decision row:** a row may only be promoted on a byte-level or black-box fixture comparison against Go; a green native test suite by itself marks the row `partial`, never `green`.

**Two traps recorded so they are not mistaken for findings:** (1) `rust/symbrain-mcp/tests/gateway_protocol_tests.rs` is 16 lines with zero `#[test]` attributes — it `include!`s `fixtures/gateway_protocol_tests_part1.rs` (10 tests) and `part2.rs` (6), so grepping the top-level file undercounts coverage to zero; (2) the 20-case MCP oracle is a Go-side determinism check (`scripts/run-go-oracle.sh HEAD run ./scripts/mcp-oracle -check`), so it proves the fixture still matches Go but says nothing about the Rust implementation on its own — the Rust side is the differential test in `oracle_differential_tests.rs`.

**Hygiene defect, not fixed here:** `migration/contract-matrix.csv` rows on lines 13 and 38 carry unquoted commas and therefore 10 and 12 fields instead of 8; editing that file with a positional rewrite must use the last field, never field 3 (a mistaken run overwrote a description and had to be restored from git). A tool that parses this file by column position will mis-read those two rows.

**skills-runner slice rescued.** The third dispatch again ended without a report and without committing; its worktree was created *nested* inside the previous worktree (`.worktrees/subagent-sa-0-a86ad2aa/.worktrees/subagent-sa-0-d2d36f7a`). The work is real this time and was preserved: committed as `37a759fb`, replayed onto `main` with the module declaration restored as `3e01d0af` on branch `migration/skills-runner-delta-20260921` in worktree `.worktrees/w-skills-runner`. It brings a genuine oracle — `scripts/skills-runner-oracle` drives the real Go `skillsrunner.Run` in isolated temp `HOME`/XDG roots, freezes six cases into `rust/symbrain-skills/tests/fixtures/runner_oracle.json`, and passes its own `-check` — plus `runner_tests.rs` with 5 green tests, three of which compare target/status/message triples against that fixture, including the 1 ns timeout case.

**Still open in that slice, dispatched as a delta (`deleg_38069bd8`) with Go line references:** `cargo clippy -p symbrain-skills --all-targets` fails with `clippy::unnecessary_wraps` on `sync_target` (a bare `cargo test` hides it); the timeout message is hardcoded instead of deriving Go's `ctx.Err()` text from a deadline built out of `opts.Timeout` (Go: `runner.go:138,163,204`); the `failure_visible_and_reported` case is asserted by shape with a comment admitting the divergence instead of matching Go's bytes; `no_legacy_binary_processes_targets` and `binary_presence_changes_nothing` have no Rust assertion; the unsupported-harness assertion uses an `||` fallback instead of Go's exact string; the fixture records no provenance.

## Resume checkpoint — 2026-09-21 (later): #626 closed, two skills-runner worker attempts rejected on measurement, Slice A re-dispatched

**#626 is closed — verified, not asserted.** On `main` @ `15d4bbad`, `cargo test -p symbrain-memory` under `scripts/run-external-env.sh` gives 21 passed / 0 failed with all seven `db_oracle_tests` green (`tables`, `memories_columns`, `indexes`, `triggers_and_views`, `lock_pragmas`, `constraint_cases`, `ordering_cases`), and `make db-memory-oracle-check` reports `PASS … (29 tables, 4 negative cases, 2 ordering cases)` — so the fixture still matches Go rather than having been tuned to the native output. `rust/symbrain-memory/tests/store_contract_tests.rs` also covers the upgrade path (`missing_shipped_columns_are_added_to_an_existing_database`, `a_native_row_reaches_the_shipped_fts_index`). The fix arrived with `2f5a5868` (#628); a dispatched worker that reported "nothing to do" was right, and the issue is now closed with that evidence.

**Skills-runner slice (Phase 7.2/7.3/7.4): two leaf-dispatch attempts, both rejected.** Both returned `status=completed` with no report block; both were verified from the worktree, not from their summaries.

- Attempt 1 (`eb50210f`, branch `hermes-subagent/subagent-sa-0-8819ebc5`): uncommitted at hand-back, and it did not compile — `sync_cli.rs` called `sync_skills()`, which was never defined (E0425).
- Attempt 2 (`c419167b`, branch `hermes-subagent/subagent-sa-0-a86ad2aa`): compiles under `cargo test`, but (a) **`cargo clippy --all-targets` is red** — this crate denies `clippy::all` + `pedantic`, so the failure is clippy-only and a bare `cargo test` hides it; (b) **no differential test was added**, while the `skills_library_has_entries()` gate was removed for the populated-library cases, i.e. the native surface was expanded with zero bytes compared to Go — the pre-existing `sync_native` tests still pass only because they exercise `claude` and `--project`, which still fall back; (c) defaults were hardcoded to `$HOME/.local/share/symskills/{library,rendered,base}` **under a doc comment claiming XDG awareness**, while Go resolves `internal/paths.SkillsDataDir()` = `$XDG_DATA_HOME/symbrain/skills` (legacy `…/symskills` only when the current dir is absent and the legacy one exists) — with `XDG_DATA_HOME` set, as every harness sets it, the native path reads the wrong library; (d) `install_skill` kept `render_target` + `install_rendered` with a comment *noting* that Go uses `RenderAll(opts.RenderDir)` + `install.Install` instead of matching it; (e) the timeout is unreachable — `Options.timeout` is stored and never used, the context is a `NoopContext` whose `is_cancelled()` is always false, so Go's `context.WithTimeout(target, 30s)` and its `sync timed out: <ctx.Err()>` message cannot be reproduced even though `internal/skillsrunner/runner_test.go`'s `TestRunTimeoutSurfacesAnError` (Timeout = 1ns) covers exactly that case.

**Failure class, named:** the leaf worker produces a structurally plausible module and documents divergences as comments instead of resolving them against the Go source; "all checks pass" referred only to `cargo test`, never to clippy, the differential, or a byte comparison. Two consequences recorded as process rules: (1) a Go fallback gate may only be removed for a case a *differential test* proves — never on the strength of a port that merely looks right; (2) every dispatch must name `cargo clippy --all-targets` explicitly, because this crate's `pedantic` denials fail clippy while `cargo test` still passes.

**Slice A re-dispatched** (`deleg_326a9291`), deliberately narrowed so a third failure stays cheap: port `Run`/`syncTarget`/`planSkill`/`installSkill`/`installOptions`/`withDefaults` into `rust/symbrain-skills` **only**, and prove it with a new frozen Go oracle (`scripts/skills-runner-oracle`, generate + `-check`) that drives the real `skillsrunner.Run` in isolated temp HOME/XDG roots and writes `rust/symbrain-skills/tests/fixtures/runner_oracle.json`, consumed by a new `tests/runner_tests.rs`. `rust/symbrain-cli/**` is explicitly out of scope: the CLI keeps its Go fallback until the fixture exists (Slice B). The brief carries the measured Go line references for the paths rule, the `RenderAll` + symlink-mode sequence, and the timeout contract.

**Next action for a resumed session:** verify Slice A from its branch (not its summary) — fixture generated by Go, `-check` clean, `runner_tests` green, clippy/fmt clean — then do Slice B: wire `sync_cli.rs` to the proven runner and delete the gate only for the cases the fixture covers, regenerating the CLI oracle fixture in the same change.
