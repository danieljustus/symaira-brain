...
def validate_evidence() -> None:
    """Require the imported evidence sidecars without treating them as green gates."""
    for name in (
        "value-signal-version.json",
        "rust009-tls-results.json",
        "rust011-cdp-probe.json",
        "rust011-value-signal.json",
    ):
        evidence = load(name)
        if not isinstance(evidence, dict):
            fail(f"{name}: evidence must be a JSON object")
...
def main() -> int:
    ...
    for item in work:
        ...
        if status == "complete" and not dependencies_complete:
            fail(f"{item['id']}: complete while a dependency is incomplete")
