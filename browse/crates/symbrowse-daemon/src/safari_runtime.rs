...
impl SafariRuntime {
    ...
    async fn find(&mut self, kind: &str, query: &str, exact: bool) -> Result<Value, String> {
        ...
        self.eval(&format!("(() => {{ const k={kind}, q={query}, exact={exact}; const text=e=>(e.innerText||e.textContent||'').trim(); const implicit=e=>{{const t=e.tagName.toLowerCase(); return t==='button'?'button':t==='a'?'link':t==='input'?'textbox':t;}}; const candidate=e=>k==='role'?(e.getAttribute('role')||implicit(e)):k==='label'?(e.getAttribute('aria-label')||text(e)):k==='placeholder'?(e.getAttribute('placeholder')||''):k==='text'?text(e):k==='testid'?(e.getAttribute('data-testid')||''):''; const matches=[...document.querySelectorAll('*')].filter(e=>{{const got=candidate(e); return exact?got===q:got.toLowerCase().includes(q.toLowerCase());}}); if (!matches.length) throw new Error('find matched no elements'); const e=matches[0], ref='safari-'+matches.indexOf(e), role=e.getAttribute('role')||implicit(e), name=e.getAttribute('aria-label')||e.getAttribute('name')||text(e), input_type=e.getAttribute('type')||'', autocomplete=e.getAttribute('autocomplete')||''; e.setAttribute('data-symbrowse-ref',ref); return {{ref,kind:k,query:q,role,name,input_type,autocomplete}}; }})()")).await
    }
}
