...
public final class VaultViewModel: ObservableObject, ModuleViewModelProtocol {
    ...
    public func generatePassword(length: Int, symbols: Bool) async {
        isGenerating = true
        defer { isGenerating = false; generatedPassword = "" }
        do {
            generatedPassword = try await client.generatePassword(length: length, symbols: symbols, profile: nil)
            createValue = generatedPassword
    ...
    public func setIntakeFiles(_ files: [URL]) {
        intakeFiles = files
        intakePreview = nil; intakeDrafts = []; intakeImportID = nil
        intakeReviewComplete = false; intakePhase = .idle
    }
    ...
    public func previewIntake() async {
        ...
        do {
            intakePreview = try await client.intakePreview(files: intakeFiles, profile: nil)
            intakeDrafts = intakePreview?.results.filter(\.isOK).map(VaultIntakeReviewDraft.init) ?? []
            intakePhase = .reviewing
        } catch { intakePhase = .failed(error.localizedDescription); report(error) }
    }
    ...
    public func stageIntake() async {
        ...
        do {
            let response = try await client.intakeStage(files: intakeFiles, ocrTexts: [:], moveToTrash: intakeMoveToTrash, profile: nil)
            intakeImportID = response.importID
    ...
    /// The UI calls this only after the human has reviewed the metadata rows.
    public func markIntakeReviewed() { intakeReviewComplete = true }

    public func promoteIntake(overwrite: Bool = false) async {
        guard let intakeImportID, intakeReviewComplete else {
            errorMessage = "Review the staged intake before promoting it."
            return
        }
        intakePhase = .promoting; clearError()
        do {
            try await client.intakePromote(importID: intakeImportID, overwrite: overwrite, profile: nil)
            intakePhase = .done
    ...
    public func clearIntake() {
        intakeFiles = []; intakePreview = nil; intakeDrafts = []; intakeImportID = nil
        intakeReviewComplete = false; intakePhase = .idle
    }
