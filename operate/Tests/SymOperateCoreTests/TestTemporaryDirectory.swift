...
func operateTestTemporaryDirectory() -> URL {
    let baseURL: URL
    ...
    if ProcessInfo.processInfo.environment["CI"]?.isEmpty != false {
        guard let rawRoot = ProcessInfo.processInfo.environment["SYMAIRA_EXTERNAL_RUNTIME_ROOT"], !rawRoot.isEmpty else {
            fatalError("SYMAIRA_EXTERNAL_RUNTIME_ROOT is required for local macOS Operate tests")
        }
        ...
        baseURL = rootURL.appendingPathComponent("operate-tests", isDirectory: true)
    } else {
        baseURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("symoperate-tests", isDirectory: true)
    }
#else
    baseURL = FileManager.default.temporaryDirectory
        .appendingPathComponent("symoperate-tests", isDirectory: true)
    ...
    do {
        ...
        fatalError("could not create Operate test root: \(error)")
    }
    return baseURL
}
